...
//  6. FIX: Guard setVapidDetails so it only runs when both keys are present.
//     Calling web-push with empty strings throws and can crash the process.
//
// SETUP (one-time):
...
// FIX: Only configure web-push when valid VAPID keys are present.
// Previously this always called setVapidDetails even with empty strings,
// which throws an error inside web-push and can crash the server on startup.
 ...
 * The frontend re-registers on every page load (fixed in PMDashboard /
 * ITDashboard) so subscriptions are always restored within one page visit.
 */
...
export const getVapidPublicKey = () => process.env.VAPID_PUBLIC_KEY || "";// src/services/pushService.js
//
// CHANGES FROM PREVIOUS VERSION:
//  1. Subscriptions now store { subscription, role } so we can push by role.
//  2. Added sendPushToRole()  — send to all users of a specific role.
